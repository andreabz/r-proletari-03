...
Se desideri consultare i dati ufficiali, rivolgiti ai [bollettini di ARPA Emilia-Romagna](https://www.arpae.it/it/temi-ambientali/aria/dati-qualita-aria).  
...
```{=html}
  ...
  # Per esecuzione in locale  
  if (nzchar(Sys.getenv("BUILD_TIMESTAMP")) == FALSE) {
    Sys.setenv(BUILD_TIMESTAMP = format(Sys.time(), "%Y-%m-%dT%H:%M:%SZ", tz = "UTC"))
  }
  
  mostra_data_build(Sys.getenv("BUILD_TIMESTAMP"))
